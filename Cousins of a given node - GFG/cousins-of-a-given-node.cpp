...
class Solution
{
    ...
    vector<int> printCousins(Node* root, Node* node_to_find)
    {
        ...
        while(!q.empty()){
            int sz = q.size();
            pair<Node*, Node*> arr[sz];
            Node* par;
            bool found = false;
            while(sz--){
                auto elt = q.front();
                arr[sz] = elt;
                Node* cur_node = arr[sz].first;
                Node* cur_par = arr[sz].second;
                q.pop();
                ...
                if(cur_node -> left != NULL)
                    q.push({cur_node -> left, cur_node});
                if(cur_node -> right != NULL)
                    q.push({cur_node -> right, cur_node});
            }
            reverse(arr, arr + sizeof(arr)/sizeof(arr[0]));
            if(found){
                for(int i = 0; i < sizeof(arr)/sizeof(arr[0]); i++){
                    if(arr[i].second != par){
                        res.push_back(arr[i].first -> data);
                    }
                }
                break;
            }
